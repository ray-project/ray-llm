...
class TextGenerationInferencePipeline(AsyncStreamingPipeline):
    ...
    # TODO Fix this
    def __call__(self, inputs: List[str | Prompt], **kwargs) -> List[Response]:
    ...
    def _parse_requests(
        ...
        for r in requests:
            ...
            parameters, stopping_parameters = self._parse_sampling_args(generate_params)
            if verbose:
                logger.info(
                    f"id: {r.id} parameters: {parameters}, stopping_parameters: {stopping_parameters} model_inputs {r.inputs}"
                )
            parsed_request = GenerationRequest(
                id=r.id,
                inputs=r.inputs,
                truncate=r.truncate,
